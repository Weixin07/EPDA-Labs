...
public class Add extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try{
                double amt = Double.parseDouble(request.getParameter("x"));
                if(amt <= 0){
                    throw new Exception();
                }
//                loginUser.setBalance(loginUser.getBalance()+amt);
//                myCustomerFacade.edit(loginUser);

                MyAccount aa = new MyAccount(amt);
                myAccountFacade.create(aa);
                
                loginUser.getMyAccount().add(aa);
                myCustomerFacade.edit(loginUser);
